...
export default function ConfirmedInternshipsPage() {
  ...
  useEffect(() => {
    if (session?.user?.role === "director" || session?.user?.role === "admin") {
      fetchConfirmedInternships();
    }
  }, [session]);

  const fetchConfirmedInternships = async () => {
    try {
      const response = await fetch("/api/internships?status=approved&isSend=1&isConfirm=1");
      if (response.ok) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">แบบฟอร์มที่ยืนยันแล้ว</h2>
        <p className="text-muted-foreground">แบบฟอร์มที่ได้รับการอนุมัติแล้ว</p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8">
              <p className="text-muted-foreground">ไม่มีแบบฟอร์มที่ยืนยันแล้ว</p>
            </div>
          </CardContent>
        </Card>
      ) : (
